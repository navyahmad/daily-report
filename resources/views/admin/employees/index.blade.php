<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('admin.employees.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-semibold shadow hover:bg-indigo-700 transition">
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200 shadow-sm">
                <form method="GET" action="{{ route('admin.employees.index') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-4">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="block sm:hidden p-3 space-y-3">
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="text-[11px] text-slate-500">{{ $employee->created_at->translatedFormat('d M Y') }}</p>
                                </div>
                                @if ($employee->is_active)
                                    <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-semibold bg-emerald-100 text-emerald-800">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-600"></span>
                                        Aktif
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-600">
                                        <span class="w-1.5 h-1.5 rounded-full bg-slate-400"></span>
                                        Nonaktif
                                    </span>
                                @endif
                            ...
                            <div class="mt-3 space-y-2 text-xs text-slate-600">
                                <div class="flex items-center justify-between gap-3">
                                    <span class="font-medium text-slate-500">Divisi</span>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-indigo-50 text-indigo-700">
                                        {{ $employee->division->name ?? '-' }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 flex gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.employees.toggle-status', $employee) }}" class="flex-1">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" 
                                            class="w-full text-xs font-semibold px-2.5 py-2 rounded-lg transition {{ $employee->is_active ? 'bg-rose-50 text-rose-600 hover:bg-rose-100' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' }}">
                                        {{ $employee->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                    </button>
                                </form>
                            </div>
                    ...
                    @empty
                        <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-xs text-slate-400">
                            Tidak ada data karyawan yang sesuai kriteria pencarian.
                        </div>
                    @endforelse
                </div>
